...
function fluxoDiario(){
  ...
  const rows=dias.map(x=>{
    ...
    const det=(x.movimentos||[]).map(m=>{
      const entrada=String(m.natureza||'')==='ENTRADA'||Number(m.valor||0)>=0;
      const alerta=m.sinal_suspeito?'<span class="dfv-status warn" title="'+esc(m.alerta_sinal||'Possível sinal incorreto')+'">⚠ Sinal suspeito</span>':'';
      return '<tr class="fluxo-mov '+(m.sinal_suspeito?'fluxo-mov-warn':'')+'"><td></td><td><strong>'+esc(m.descricao||'Lançamento')+'</strong><small>'+esc(m.fornecedor||'')+(m.categoria?' · '+esc(m.categoria):'')+(m.transferencia_interna?' · transferência interna':'')+'</small>'+alerta+'</td><td>'+ (entrada?'Entrada':'Saída') +'</td><td class="'+(entrada?'fluxo-ok':'fluxo-bad')+'">'+brl(Math.abs(Number(m.valor||0)))+'</td><td>'+ (m.saldo_antes==null?'—':brl(m.saldo_antes)) +'</td><td>'+ (m.saldo_depois==null?'—':brl(m.saldo_depois)) +'</td><td colspan="2"></td></tr>';
    }).join('');
...
function css(){
  ...
  .fluxo-mov-warn td{background:#fff7ed!important}.fluxo-mov .dfv-status{margin-top:4px;display:inline-flex}
