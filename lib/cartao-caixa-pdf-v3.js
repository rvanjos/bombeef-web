...
function classificarMovimento(desc, natureza, secao) {
  const s = limpar(desc).toUpperCase();
  if (natureza === 'C') {
    if (/DESCONTO/.test(s)) return 'DESCONTO';
    if (/ESTORNO|REVERS[AÃ]O|REVERSAO/.test(s)) return 'ESTORNO';
    if (/CASHBACK/.test(s)) return 'CASHBACK';
    if (/ABATIMENTO|BONIFICA|CR[EÉ]DITO/.test(s)) return 'ABATIMENTO';
    if (/ANUIDADE/.test(s)) return 'CRÉDITO ANUIDADE';
    return 'CRÉDITO';
  }
  if (/ANUIDADE/.test(s)) return 'ANUIDADE';
  if (/IOF/.test(s)) return 'IOF';
  if (/JUROS/.test(s)) return 'JUROS';
  if (/ENCARGO/.test(s)) return 'ENCARGO';
  if (/TARIFA/.test(s)) return 'TARIFA';
  if (/PARCELAD/.test(String(secao || '')) || /\b\d{1,2}\s+DE\s+\d{1,2}\b/i.test(s)) return 'COMPRA PARCELADA';
  return 'COMPRA';
}
...
function interpretarLinhasFinanceiras(linhas, cab) {
  ...
  for (const linha of linhas) {
    ...
    // Alguns ajustes financeiros não possuem data impressa. Eles precisam
    // permanecer na fatura porque podem aumentar ou reduzir o saldo devido.
    if (!data && !/(ANUIDADE|TARIFA|IOF|ENCARGO|JUROS|DESCONTO|ESTORNO|REVERS[AÃ]O|CASHBACK|ABATIMENTO|BONIFICA|CR[EÉ]DITO)/i.test(desc)) continue;

    const sinal = fim.natureza === 'C' ? -1 : 1;
    const valor = Number((fim.valor * sinal).toFixed(2));
    const movimento = classificarMovimento(desc, fim.natureza, secao);
    const item = {
      ...
      natureza_cartao: fim.natureza,
      movimento,
      efeito: fim.natureza === 'C' ? 'ABATE_FATURA' : 'AUMENTA_FATURA',
  ...
  const cartoes = [...grupos.values()].filter(g => g.itens.length).map(g => {
    ...
    for (const it of g.itens) {
      // O sinal faz parte da chave: uma cobrança e um crédito do mesmo valor
      // e descrição são movimentos distintos e nunca podem se anular por dedupe.
      const k = `${it.data_original || ''}|${limpar(it.descricao)}|${Number(it.valor).toFixed(2)}|${it.natureza_cartao}`;
      if (vistos.has(k)) continue;
      vistos.add(k);
      itens.push({ ...it, hash_item: hash([g.final, cab.competencia, it.data_compra, it.descricao, Number(it.valor.toFixed(2)), it.natureza_cartao, itens.length]) });
    }
...
module.exports = { interpretarCaixaPdfV3, agruparLinhas, acharCabecalho, interpretarLinhasFinanceiras, classificarMovimento, numBR, dataISO };
