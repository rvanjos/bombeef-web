...
function normalizarCabecalhoCaixa(texto) {
  return String(texto || '')
    .replace(/\bVENCIMENTO\s+(\d{2}\/\d{2}\/\d{4})\b/i, 'VENCIMENTO\n$1')
    .replace(/\bVALOR TOTAL DESTA FATURA\s+R\$\s*([\d.]+,\d{2})\b/i, 'VALOR TOTAL DESTA FATURA\nR$ $1')
    .replace(/\bVALOR TOTAL DESTA FATURA\s+([\d.]+,\d{2})\b/i, 'VALOR TOTAL DESTA FATURA\nR$ $1');
}
...
async function extrairPdfJsVariantes(buf, password='') {
  ...
  try {
    for (let i=1;i<=doc.numPages;i++) {
      ...
      // Na página 1 mantemos o cabeçalho completo, mas normalizamos vencimento e
      // total em linhas separadas (formato esperado pelo parser). Nas demais,
      // preservamos só a coluna direita, onde ficam demonstrativo e transações.
      if (i === 1) {
        caixaColuna.push(normalizarCabecalhoCaixa(linhasPagina.join('\n')));
      } else {
        caixaColuna.push(montarLinhasColuna(tc.items, 295).join('\n'));
      }
    }
  } finally {
