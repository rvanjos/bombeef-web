...
const TABELAS_POR_MODULO = {
  ...
  pessoas: [
    ...
    'ponto_registros','ponto_auditoria','ponto_jornada_dia','login_sessoes'
  ],
  administracao: ['auditoria_eventos','config_sistema','validade_acoes','perdas']
};

const COMPARTILHADAS = new Set([
  'empresas','lojas','usuario_lojas','usuarios','categorias_dre','fornecedores_lookup',
...
async function auditarSegurancaMultiloja(pool, { aplicarPoliticas = false } = {}) {
  ...
  try {
    ...
    `);
    comLoja.forEach(r => esperadas.add(r.table_name));

    for (const tabela of [...esperadas].sort()) {
      if (!publicSet.has(tabela)) continue;
      ...
      const item = {
        ...
        tem_indice: temIndice || (aplicarPoliticas && !!lojaCol), rls_ativo: rlsAtivo || (aplicarPoliticas && !!lojaCol && lojaNotNull),
...
module.exports = { TABELAS_POR_MODULO, auditarSegurancaMultiloja, aplicarPoliticaFailClosed };
