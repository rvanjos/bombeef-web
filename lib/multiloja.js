...
async function garantirEstruturaMultiloja(pool, { operacional = false } = {}) {
  ...
  try {
    ...
    const versaoAdministracao = '2026-09-administracao-v1';
    const migracaoAdministracaoAplicada = operacional && (await client.query(
      `SELECT 1 FROM multiloja_migracoes WHERE versao=$1`,
      [versaoAdministracao]
    )).rowCount > 0;
    ...
    const tabelasEstoqueCompras = [
      ...
      'validade_internos','validade_interno_anexos','validade_documentos','movimentos_estoque'
    ];
    ...
    const tabelasFinanceiro = [
      ...
      'faturamento_metas','metas','dre_sessoes','dre_lancamentos','dre_fechamentos','dre_fechamento_eventos','dre_fluxo_saldo_controle','dre_fluxo_conferencias',
    ...
    if (operacional && !migracaoProducaoAplicada) {
      const tabelasProducaoComercial = [
        ...
        'pagamentos_retiradas','pagamento_retirada_itens'
      ];
    ...
    if (operacional && !migracaoAdministracaoAplicada) {
      const tabelasAdministracao = [
        'auditoria_eventos','config_sistema','validade_acoes','perdas'
      ];
      for (const tabela of tabelasAdministracao) {
        const existe = await client.query(`SELECT to_regclass($1) AS tabela`, [`public.${tabela}`]);
        if (!existe.rows[0].tabela) continue;
        await client.query(`ALTER TABLE ${tabela} ADD COLUMN IF NOT EXISTS loja_id INTEGER REFERENCES lojas(id) ON DELETE RESTRICT`);
        await client.query(`ALTER TABLE ${tabela} ALTER COLUMN loja_id SET DEFAULT bb_loja_padrao()`);
      ...
      if ((await client.query(`SELECT to_regclass('public.config_sistema') AS t`)).rows[0].t) {
        await client.query(`ALTER TABLE config_sistema DROP CONSTRAINT IF EXISTS config_sistema_pkey`);
        await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS uq_config_sistema_loja_chave ON config_sistema(loja_id,chave)`);
      }
