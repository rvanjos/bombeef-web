...
module.exports = function (pool, app) {
  ...
  r.use((req, res, next) => {
    ...
    res.json = (body) => {
      if (body?.ok !== false && ['POST','PUT','DELETE','PATCH'].includes(req.method)) {
        _pub('validade', { type: 'validade_atualizado' });
      }
      return orig(body);
    };
    next();
  ...
  async function atualizarStatus() {
    const res = await pool.query(`
      UPDATE validade_items SET
        ...
        END,
        atualizado_em = NOW()
      WHERE status NOT IN ('descartado','vendido','devolucao') AND dt_resolucao IS NULL
  ...
  r.get('/kpis', async (req, res) => {
    try {
      ...
      const { rows } = await pool.query(`
        ...
        FROM validade_items
        WHERE status NOT IN ('descartado','vendido','devolucao') AND dt_resolucao IS NULL OR data_validade >= DATE_TRUNC('month', NOW())
      `);
  ...
  r.get('/', async (req, res) => {
    try {
      ...
      conds.push(`status NOT IN ('descartado','vendido','devolucao') AND dt_resolucao IS NULL`);
  ...
  r.get('/alertas-confirmacao', async (req, res) => {
    try {
      ...
      const { rows } = await pool.query(`
        ...
        WHERE v.status NOT IN ('descartado','vendido','devolucao') AND dt_resolucao IS NULL
    ...
    try {
      ...
      const { rows: semCod } = await pool.query(
        `SELECT id, descricao FROM validade_items
         WHERE (codigo IS NULL OR codigo = '') AND status NOT IN ('descartado','vendido','devolucao') AND dt_resolucao IS NULL`
      );
  ...
  r.delete('/:id', async (req, res) => {
    ...
    // Mesmo mapeamento da rota em lote (/baixa): só 'vencimento' vira 'descartado'
    // (que é o que dispara a geração de perda). Os demais motivos ficam como estão —
    // o item some da lista ativa por causa do dt_resolucao, não do status.
    const motivo = resolucao || 'outro';
    const novoStatus = motivo === 'vencimento' ? 'descartado' : motivo;
    try {
      await pool.query(
        `UPDATE validade_items SET
  ...
  r.get('/historico', async (req, res) => {
    try {
      ...
      const conds = [`(status IN ('descartado','vendido','devolucao') OR dt_resolucao IS NOT NULL OR status = 'vencido')`], params = [];
      ...
      const conds2 = [`(status IN ('descartado','vendido','devolucao') OR dt_resolucao IS NOT NULL OR status = 'vencido')`], params2 = [];
      if (resolucao && resolucao !== 'todos') {
        if (resolucao === 'vencimento') {
