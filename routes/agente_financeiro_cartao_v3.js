    ...
    for (const cartao of preview.cartoes) {
      ...
      for (const it of cartao.itens) {
        ...
        await inserirDinamico(client,'cartao_fatura_itens',{
          loja_id:req.user.lojaId, fatura_id:faturaId, data_compra:it.data_compra, descricao:it.descricao,
          valor:it.valor, categoria_dre:null, portador:cartao.portador, hash_item:it.hash_item, removido:false
        });
        itensCriados++;
      }
